...
Estimate the effect of job training grant (*grant*) on the hours of job training per employee (*hrsemp*). This simple relation can be summarized by the equation $hrsemp = \beta_0 +\beta_1grant+u$. The OLS results are reported below.
...
jtrain_87<-subset(jtrain,year<1988)
# run a plain ols
ols <- lm(hrsemp ~ grant, data = jtrain_87) # run regression
...
In a panel context, the previous relation can be summarized by the following equation: $hrsemp_{it} = \beta_0 +\beta_1grant_{it}+a_i+u_{it}$, t=1987, 1988, where $a_i$ is term for unobserved heterogeneity.
...
The first difference equation is characterized by $\Delta hrsemp_i = \beta_0 +\beta_1 \Delta  grant_i+\Delta u_i$
...
pdim(jtrain.p)
# run FD model
fd <- plm(hrsemp ~ grant, data = jtrain.p,
...
fd_full <- plm(hrsemp ~ grant, data = jtrain,
              index = c("fcode", "year"), model = "fd")
fe_within_full <- plm(hrsemp ~ grant, data = jtrain,
              index = c("fcode", "year"), model = "within")
...
Strict exogeneity implies that $u_{it}$ should not correlate with the independent variables of all time periods. A serial correlation test allows to identify if such assumption holds. The results below reject the null of serial correlation for the FD model but not for the FE model. This corroborates the results found in *v)*.  
...
The model to estimate either using FE or FD then becomes $hrsemp_{it} = \beta_0 +\beta_1grant_{it}+ \beta_1union_{it}+ a_i+u_{it}$, t=1987, 1988, 1989.

```{r q1viii_full, echo=FALSE, message=FALSE, warning=FALSE,results='asis', float = FALSE}
# run a plain full panel
fd_full_union <- plm(hrsemp ~ grant + union, data = jtrain,
              index = c("fcode", "year"), model = "fd")
fe_within_full_union <- plm(hrsemp ~ grant + union, data = jtrain,
              index = c("fcode", "year"), model = "within")
